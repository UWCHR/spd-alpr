...
df <- df %>%
  mutate(device = factor(device) %>%
           fct_recode(
                        "A" = "SPD2020621MDC", "B" = "SPD2020681MDC", 
                         "C" = "M207709", "D" = "M207541_Deleted_2021-10-21_22 50:42",
                         "E" ="SPD2020602MDC", "F" = "PEOATG508533", 
                         "G" = "PEO508459", "H" = "SPD2020537MDC", 
                         "I" = "BXH9904 SPD2020627MDC"))
...
devices_missing_address = reads_missing_address %>%
  filter(device== "H" | device == "I")  %>%
  nrow()
devices_missing_address_p = devices_missing_address / total_reads * 100

peo_device_reads = sum(df$device== "G" | df$device == "F") 
...
Our preliminary statistical analysis suggests a negative correlation between plate hits and median household income, indicating a possible increase of surveillance in lower income neighborhoods. **Figure 12** below depicts this relationship; the darker the blue, the higher the intensity of ALPR reads. 

However, when accounting for over-dispersion, the correlation loses its statistical significance. This finding should thus be interpreted with caution. Moreover, our analysis only covers data from a single week, and a large portion of the collected readings are missing important location data. 
...
medincome_p1 <- medincome_boundaries_df + 
  ...
  labs(
       subtitle = "The data indicates ALPR reads are negatively correlated with median income",
...
We did not identify any other suggestive correlations between SPD surveillance and other demographic factors, but [link to our notebook and how they can see some analyses there?]. 
 
# Preliminary statistical analysis

We attempted a statistical analysis of the relationship between plate read frequency and socioeconomic indiciators including estimated median household income and race/ethnicity per zipcode. Preliminary Poisson modeling showed evidence of a negative correlation between plate read frequency and median household income; however, after post-fit adjustments for overdispersion the results are not statistically significant. For a brief analysis, see this [companion notebook](analyze.html).
