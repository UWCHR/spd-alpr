...
```{r overview and maps, echo=FALSE, fig.align='center', message=FALSE, warning=FALSE, include=FALSE}
...
Our data set contains `r total_reads` instances where license plates were detected by SPD's surveillance system from `r dtrange`. Figure 1 provides an overview of the geographic distribution of these reads. Nearly all of the non-Seattle reads are located along the journey to a prison north of Seattle. For a closer look, Figure 2 depicts a zoomed-in map and highlights areas with the highest concentration of ALPR reads through brighter colors. The most read-dense areas are located near SPD precincts. Notably, there are no reads in West Seattle and very few reads in East Capitol Hill and waterfront neighborhoods like Magnolia.
...
The dataset consists of `r device_count` devices. Each device captured an average of `r `avg_reads_per_device` license plates, with the most device capturing `r max_reads_per_device` plates. Two devices were not accompanied by any addresses, comprising `r round(devices_missing_address_p, 0)`% of the reads in the data. Another two devices were attached to parking enforcement officers (PEO). Reads from PEO devices account for % of reads in the dataset. 


Of the license plates captured, `r one_read plates` reads, which accounts for `r round(one_read_p, 0)`% of the total reads, were only captured once. We also found that on average, each *unique* license plate was captured 'r round(avg_reads, 2)' times.

```{r describe date and time, echo=FALSE, message=FALSE, include=FALSE}
...
`r hist_reads_per_hour`
`r hist_reads_per_day`

```{r detailed maps, echo=FALSE, message=FALSE, warning=FALSE}
