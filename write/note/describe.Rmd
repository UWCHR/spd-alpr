...
Figure 1 provides an overview of the geographic distribution of these reads. Nearly all of the non-Seattle reads are located along the journey to a prison north of Seattle. For a closer look, Figures 2, 3, and 4 depict zoomed-in maps that highlights areas with the highest concentration of ALPR reads through brighter colors. The most read-dense areas are located near SPD precincts. Notably, there are no reads in West Seattle and very few reads in East Capitol Hill and waterfront neighborhoods like Magnolia.
...
The data set consists of `r device_count` devices. Each device captured an average of `r avg_reads_per_device` license plates, with the most device capturing `r max_reads_per_device` plates. Two devices were completely missing addresses, comprising `r round(devices_missing_address_p, 0)` % of all reads in the data. Another two devices were attached to parking enforcement officer vehicles (PEO). Reads from PEO devices account for  `r round(peo_device_reads_p, 0)`% of reads in the data set. 
...
#Need to check with Phil if geom_line OR geom_path is better; R said geom_line is more useful for time series, but my research says the opposite?
...
device_M207541_map <- ggmap(base) + 
  geom_line(data = device_M207541, aes(x = lon, y = lat, color = hour(dt), 
                                       group = device)) + 
  facet_wrap(~day(dt), strip.position = "bottom",  ncol = 3) +
  theme_void() + 
   theme(
     plot.caption = element_text(face = "italic"), 
